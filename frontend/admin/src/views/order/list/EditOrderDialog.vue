<template>
  ...
  >
    ...
    <div class="order-info-bar">
      ...
      <span class="product-name">{{ orderData?.productName }}</span>
    </div>
    ...
    >
      <el-row :gutter="20">
        <el-col :span="12">
          <ElFormItem label="收货人" prop="receiptName">
            <ElInput v-model="formData.receiptName" placeholder="请输入收货人姓名" maxlength="20" />
          </ElFormItem>
        </el-col>
        <el-col :span="12">
...
import { changeOrder, type OrderListItem } from '@/api/order'

declare const BMap: any
declare const BMAP_STATUS_SUCCESS: number

defineOptions({ name: 'EditOrderDialog' })
...
watch(
  ...
  (newOrder) => {
    if (newOrder) {
      const data = {
        receiptName: newOrder.receiptName || '',
        phone: newOrder.phone || '',
        receiptAddress: newOrder.receiptAddress || ''
      }
      formData.value = { ...data }
...
const handleSubmit = async () => {
  if (!formRef.value || !props.orderData) return
  ...
  try {
    ...
    const trimmedName = formData.value.receiptName?.trim() || ''
    const trimmedAddress = formData.value.receiptAddress?.trim() || ''
    
    // 空白字符校验
    if (formData.value.receiptName && !trimmedName) {
      ElMessage.warning('收货人名称不能只包含空白字符')
      return
    }
    if (formData.value.receiptAddress && !trimmedAddress) {
    ...
    // 判断是否有修改
    const isNameChanged = trimmedName !== (originalData.value.receiptName?.trim() || '')
    const isPhoneChanged = trimmedPhone !== (originalData.value.phone?.trim() || '')
    const isAddressChanged = trimmedAddress !== (originalData.value.receiptAddress?.trim() || '')
...
.order-info-bar {
  ...
  .order-id { color: var(--el-color-primary); font-family: monospace; font-weight: 500; }
